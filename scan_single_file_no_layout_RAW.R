...
ui <- fluidPage(
    ...
    # tags$head( 
    #     tags$style(
    #         HTML(  ".content {  background-color:white; padding-top: 40px; padding-left: 40px;}
    ...
    # Buffer polygons?
    # "For large datasets, the Cs calculus may take a long time. The use of inner buffers in selected area-size-classes (quartiles) may save precious time,  avoiding marginal spatial overlaps." 
    # "SCAN algorithm uses original distribution areas but some overlaps may be discarded in the Cs table because of buffers. Be careful to not loose information!"
    # "Obs. Buffers are better suited for metric-based CRS projections. The map tab allows crs change (e.g. from WGS84:4326 'degree' unit to WGS84:3857 Pseudo-Mercator 'metre'."
...
server <- function(input, output) {# source
    { # source
