...
output$invalid_map_species <- renderTable( map2()[invalid_map(), ] |>  st_drop_geometry() |> select(sp) |> summarise(species = paste(sp, collapse = ', '))  )
...
output$map_shp_names <- renderText(   paste("Column names are:    ", paste(map() |> names(), collapse = '    /    '), sep = '\n'))
